    ...
    const imprimirArqueo = () => {
        ...
        win.document.write(`
            ...
            <body>
                ...
                <div class="sep"></div>
                <div class="row"><span>Fondo de Apertura</span><span class="bold">Bs. ${formatCurrency(resumenCierre.apertura)}</span></div>
                <div class="sep-dot"></div>
                <div class="row"><span>Ventas en Efectivo</span><span class="bold">+Bs. ${formatCurrency(resumenCierre.ventas_efectivo)}</span></div>
                <div class="sep-dot"></div>
                <div class="row"><span>Ventas Tarjeta/Transf.</span><span class="bold">Bs. ${formatCurrency(resumenCierre.ventas_tarjeta)}</span></div>
                <div class="sep-dot"></div>
                <div class="total-row"><span>EFECTIVO ESPERADO</span><span>Bs. ${formatCurrency(resumenCierre.efectivo_esperado)}</span></div>
                <div class="total-row" style="border-top:none"><span>EFECTIVO CONTADO</span><span>Bs. ${formatCurrency(resumenCierre.efectivo_real)}</span></div>
                <div class="box">
                    <div style="font-size:12px">DIFERENCIA</div>
                    <div class="amount">${resumenCierre.diferencia > 0 ? '+' : ''}Bs. ${formatCurrency(resumenCierre.diferencia)}</div>
                    ...
                    <div style={{ backgroundColor: 'var(--bg-card)', padding: '2rem', border: '2px solid var(--primary)', borderRadius: 'var(--radius)' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                            <div>
                                ...
                                <div style={{ fontSize: '1.4rem', fontWeight: 'bold' }}>Bs. {formatCurrency(resumenCierre.apertura)}</div>
                            </div>
                            <div>
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Ventas en Efectivo</div>
                                <div style={{ fontSize: '1.4rem', fontWeight: 'bold' }}>Bs. {formatCurrency(resumenCierre.ventas_efectivo)}</div>
                            </div>
                            <div>
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Efectivo Esperado</div>
                                <div style={{ fontSize: '1.4rem', fontWeight: 'bold', color: 'var(--primary)' }}>Bs. {formatCurrency(resumenCierre.efectivo_esperado)}</div>
                            </div>
                            <div>
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Efectivo Real (Contado)</div>
                                <div style={{ fontSize: '1.4rem', fontWeight: 'bold' }}>Bs. {formatCurrency(resumenCierre.efectivo_real)}</div>
                            </div>
                            <div>
                                ...
                                <div style={{ fontSize: '1.4rem', fontWeight: 'bold', color: resumenCierre.diferencia === 0 ? 'var(--success)' : 'var(--danger)' }}>
                                    Bs. {formatCurrency(resumenCierre.diferencia)}
                                </div>
                            </div>
                        </div>
                    ...
                    {!cajaAbierta ? (
                        <div style={{ backgroundColor: 'var(--bg-card)', padding: '2rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)', textAlign: 'center' }}>
                            ...
                            <form onSubmit={handleAbrir} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', textAlign: 'left', maxWidth: '400px', margin: '0 auto' }}>
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '600' }}>Monto de Apertura (Bs.)</label>
                                    <input 
                                        type="number" 
                                        step="0.01"
                                        placeholder="0.00"
                            ...
                            {cierreStep === 'form' && (
                                <div style={{ backgroundColor: 'var(--bg-card)', padding: '2rem', borderRadius: 'var(--radius)', border: '1px solid var(--primary)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', backgroundColor: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: 'var(--radius)', marginBottom: '1.5rem' }}>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                                ...
                                                <span style={{ fontWeight: '600' }}>Bs. {formatCurrency(cajaAbierta.monto_apertura)}</span>
                                            </div>
                                        </div>
                                    ...
                                    <form onSubmit={handleVerificarArqueo} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                        <div>
                                            <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '600' }}>Efectivo Real en Caja (Bs.)</label>
                                            <input 
                                                type="number" 
                                                step="0.01"
                                ...
                                <div style={{ backgroundColor: 'var(--bg-card)', padding: '2.5rem', borderRadius: 'var(--radius)', border: `2px solid ${previewData.diferencia === 0 ? 'var(--success)' : 'var(--danger)'}` }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                                        <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '1.25rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                                            ...
                                            <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Bs. {formatCurrency(previewData.apertura)}</div>
                                        </div>
                                        <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '1.25rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                                            <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '0.3rem' }}>Ventas en Efectivo</div>
                                            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--success)' }}>+Bs. {formatCurrency(previewData.ventas_efectivo)}</div>
                                        </div>
                                        <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', padding: '1.25rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                                            <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '0.3rem' }}>Ventas Tarjeta / Transf.</div>
                                            <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Bs. {formatCurrency(previewData.ventas_tarjeta)}</div>
                                        </div>
                                        <div style={{ backgroundColor: 'rgba(59, 130, 246, 0.08)', padding: '1.25rem', borderRadius: 'var(--radius)', border: '1px solid rgba(59, 130, 246, 0.3)' }}>
                                            ...
                                            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>Bs. {formatCurrency(previewData.efectivo_esperado)}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div style={{ 
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                            <div>
                                                ...
                                                <div style={{ fontSize: '1.8rem', fontWeight: 'bold' }}>Bs. {formatCurrency(previewData.efectivo_real)}</div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: 'var(--primary)' }}>Bs. {formatCurrency(previewData.efectivo_esperado)}</div>
                                            </div>
                                        </div>
                                        <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
                                            ...
                                            <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: previewData.diferencia === 0 ? 'var(--success)' : 'var(--danger)' }}>
                                                {previewData.diferencia > 0 ? '+' : ''}Bs. {formatCurrency(previewData.diferencia)}
                                            </div>
                                        </div>
                                    </div>
