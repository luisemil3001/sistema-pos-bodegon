...
const TopCustomersPage = () => {
  ...
  const { fetchTopCustomers } = useReports();

  const loadData = async () => {
    setLoading(true);
    setError(null);
    try {
      const result = await fetchTopCustomers(filters);
      setData(result);
    } catch (err) {
            ...
            {data.length === 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{
                  ...
                  <tr>
                    <th style={{ padding: '1rem', textAlign: 'left', color: 'var(--text-main)' }}>Cliente</th>
                    <th style={{ padding: '1rem', textAlign: 'left', color: 'var(--text-main)' }}>RNC</th>
                    <th style={{ padding: '1rem', textAlign: 'center', color: 'var(--text-main)' }}>Facturas</th>
                ...
                <tbody>
                  {data.map((item, index) => (
                    <tr key={index} style={{
                      ...
                      <td style={{ padding: '1rem', color: 'var(--text-muted)', fontSize: '0.9rem' }}>{item.rnc}</td>
                      ...
                      <td style={{ padding: '1rem', textAlign: 'right', color: 'var(--text-main)' }}>
                        ${formatCurrency(item.total_compras_usd)}
                      </td>
                      <td style={{ padding: '1rem', textAlign: 'right', color: 'var(--text-main)' }}>
                        Bs. {formatCurrency(item.total_compras_bs)}
                      </td>
                    </tr>
                  ))}
                </tbody>
